...
def run_hammer(corrected_dataset_yaml_filename, configs_dir, execution_home, cfg,
    ...
    if not only_compressing_is_needed:
        ...
        if cfg.iontorrent:
            copy_tree(os.path.join(configs_dir, "ionhammer"), dst_configs, preserve_times=False)
            cfg_file_name = os.path.join(dst_configs, "ionhammer.cfg")
        else:
            copy_tree(os.path.join(configs_dir, "hammer"), dst_configs, preserve_times=False)
