...
def print_used_values(cfg, log):
    ...
    if "dataset" in cfg:
        ...
        log.info("  Reads:")
        dataset_data = pyyaml.load(open(cfg["dataset"].yaml_filename, 'r'), Loader=pyyaml.FullLoader)
    ...
    try:
        ...
        if not os.path.isdir(tmp_configs_dir):
            if options_storage.configs_dir:
                copy_tree(options_storage.configs_dir, tmp_configs_dir, preserve_times=False, preserve_mode=False)
            else:
                copy_tree(os.path.join(spades_home, "configs"), tmp_configs_dir, preserve_times=False, preserve_mode=False)
